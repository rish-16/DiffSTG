...
def write_pdb(init_path, struct, specification, lines):
    with open(init_path + struct+specification+'.pdb', 'w') as of:
        for line in lines:
            of.write(line+'\n')
...
if __name__=="__main__":
    ...
    parser.add_argument("-o", "--outpath", required=True, help="save path for PDB")
    ...
    struct = args.struct
    residue_Map, typeMap, nameMap = get_maps("./Maps/")
    if args.datasetMD is not None:
        ...
        # print('Generating pdb for MD dataset for '+struct+' frame '+str(frame))
        ...
        write_pdb(f"{args.outpath}/", struct, '_MD_frame' + str(frame), lines)
